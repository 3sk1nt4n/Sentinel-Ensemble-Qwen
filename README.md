...
| Demonstration video (< 3 min) | built - host + link | [`docs/sentinel-qwen-demo.mp4`](docs/sentinel-qwen-demo.mp4) - 2:44, title-card intro + the 0-vs-4 two-tier reveal, real run output from both runs. Upload to **YouTube (Public)** and put the link on the Devpost form (the one open submission step) |
