...
| Demonstration video (< 3 min) | done | **[youtu.be/NV6Zn0YrD1w](https://youtu.be/NV6Zn0YrD1w)** (YouTube, Public) - also in-repo [`docs/sentinel-qwen-demo.mp4`](docs/sentinel-qwen-demo.mp4) - 2:56, title-card intro + the 0-vs-4 two-tier reveal + real run output from both runs + the on-camera Proof-of-Alibaba-Cloud beat (the Workbench screenshot: SAS instance Running, live SENTINEL-QWEN-OK), closing on the spelled-out "Digital Forensics & Incident-Response (DFIR)" card (Built by Adil Eskintan). *(Run footage predates the product's rename - on-screen banners read "SIFT Sentinel", the engine's former name.)* The same link goes on the Devpost form |
