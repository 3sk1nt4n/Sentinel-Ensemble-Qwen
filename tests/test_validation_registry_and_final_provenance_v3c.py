...
def test_validation_family_registry_v3c_has_only_complete_fact_specs():
    ...
    for family, spec in reg.items():
        ...
        assert spec.get("family") == family
        assert spec.get("fact_type")
        assert spec.get("roles"), (family, spec)
        assert spec.get("claim_types"), (family, spec)

    ps = reg["powershell_command_fact"]
    assert "parse_event_logs" in set(ps.get("source_tools") or ps.get("producer_tools") or [])
    assert "powershell_command" in set(ps["claim_types"])


def test_final_finding_sanitizer_removes_internal_provenance_everywhere(tmp_path: Path):
    state = tmp_path
    (state / "finding_disposition_buckets.json").write_text(json.dumps({
        "confirmed_malicious_atomic": [{
            ...
            "claims": [
                ...
                {"type": "raw", "text": "reference_set fallback"},
