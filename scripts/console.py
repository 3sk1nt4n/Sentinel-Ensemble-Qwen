...
# Runnable from a fresh clone (no install): the package lives under src/.
...
EVIDENCE_DIR = Path("/cases/evidence")
EVIDENCE_GLOBS = ("*.img", "*.E01", "*.raw", "*.e01")
OLLAMA_URL = os.environ.get("SIFT_OLLAMA_URL", "http://127.0.0.1:11434/api/chat")
...
DISK_TOOLS = {"get_amcache", "extract_mft_timeline", "parse_event_logs", "parse_shellbags"}
...
class SIFTConsole:
    ...
    def cmd_findings(self) -> None:
        ...
        data = load_state("findings_final.json")
        if not data:
    ...
    def cmd_show(self, finding_id: str) -> None:
        ...
        if n_types == 1:
            types_str += " only"
        panel_lines = [
            f"[bold]{match.get('finding_id')}[/bold]: {match.get('artifact', '')}",
        ...
        console.print(Panel("\n".join(panel_lines), title=finding_id))
    ...
    def cmd_investigate(self, pid_str: str) -> None:
        ...
        # Connections
        conns = [c for c in self._get_netscan() if c.get("PID") == target_pid]
        if conns:
    ...
    def cmd_connections(self, ip: str) -> None:
        ...
        matches = [
            c for c in netscan
            if c.get("ForeignAddr") == ip or c.get("LocalAddr") == ip
        ]
        if not matches:
...
def render_welcome(mode: str) -> None:
    ...
    mode_label = {
        ...
        "live": f"LIVE (Anthropic: {ANTHROPIC_MODEL})",
    }
    ...
    if evidence:
        ...
        lines.append("  [dim]No evidence files detected in /cases/evidence/[/dim]")
